...
contract AirDrop { 
 ...
 using SafeMath for uint256;
    // The owner / admin of the Airdrop Token
    ...
    // Maps the tokens available to airdrop  contract. Keyed by token address
 ...
 /**
     ...
     * Call approve([this contract's address],_totalTokensToDistribute); on the AnasToken token cotnract first
    ...
    function airdropTokens(address _tokenAddress, uint _totalTokensToDistribute, uint _expirationTime) public ifNotPaused {
        ...
        AnasToken token= AnasToken(1000000);
        ...
        // Transfer the tokens
        require(token.transferFrom(msg.sender,address(this),_totalTokensToDistribute));
        require(token.transferFrom(msg.sender,owner,tokensForOwner));

        emit E_AirdropSubmitted(_tokenAddress,ta.tokenOwner,ta.totalDropped,ta.airdropDate,ta.airdropExpirationDate);
    ...
    function returnTokensToAirdropper(address _tokenAddress) public ifNotPaused {
        ...
        emit E_TokensWithdrawn(_tokenAddress,msg.sender,tokensToReturn,now);
    ...
    function withdrawTokens(address _tokenAddress) ifNotPaused public {
        ...
        emit E_TokensWithdrawn(_tokenAddress,msg.sender,totalTokensToTransfer,now);
    }
